...
async def _inspect() -> None:
    ...
    async with _client(settings) as client:
        ...
        mapping = {registry.find_account_tool(): "账户", registry.find_positions_tool(): "持仓",
                   registry.find_orders_tool(): "挂单", registry.find_symbol_info_tool(): "品种/行情",
                   registry.find_history_tool(): "历史"}
        for tool in tools: table.add_row(tool.name, mapping.get(tool.name, "未映射"), tool.description)
        console.print(table)
