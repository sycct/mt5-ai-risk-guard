...
DeepSeek 只负责解释。其等级不能低于硬规则等级；无 Key、网络失败或连续两次非 JSON 时自动使用硬规则报告。缺失的 MCP 能力/字段会明确列出并保留为 `None`，不会编造。
...
客户端要求 `json_object` 并自动重试一次；仍失败则降级为硬规则报告，监控不会因此停止。也应确认所选模型支持 JSON 输出；模型名由服务端实际可用型号决定。
